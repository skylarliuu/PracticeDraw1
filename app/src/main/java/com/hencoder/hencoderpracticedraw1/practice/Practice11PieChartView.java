...
public class Practice11PieChartView extends View {
    ...
    private RectF rect;
    private Paint mPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
    private float startAngel,sweepAngel,lineAngel = 0f;
    private float lineStartX,lineEndX,lineStartY,lineEndY = 0f;
    ...
    private void init() {
        ...
        for(Data data1:datas){
            maxNum = Math.max(data1.getNumber(),maxNum);
            total += data1.getNumber();
        }
        rect = new RectF(-300,-300,300,300);
...
//        练习内容：使用各种 Canvas.drawXXX() 方法画饼图
        ...
        for(Data data:datas){
            ...
            sweepAngel = data.getNumber() / total * 360f;
            lineAngel = startAngel + sweepAngel / 2;//扇形中点对应的角度
            lineStartX = (float) (radius * Math.cos(lineAngel / 180f * Math.PI));
            lineStartY = (float) (radius * Math.sin(lineAngel / 180f * Math.PI));
            lineEndX = (float) ((radius + 50) * Math.cos(lineAngel / 180f * Math.PI));
            lineEndY = (float) ((radius + 50) * Math.sin(lineAngel / 180f * Math.PI));

            //画扇形
            if(data.getNumber() == maxNum){//最大的扇形要突出一点
                canvas.save();
                canvas.translate(lineStartX * 0.1f,lineStartY * 0.1f);
                canvas.drawArc(rect,startAngel,sweepAngel,true,mPaint);
            }else{
                canvas.drawArc(rect,startAngel,sweepAngel - 2f,true,mPaint);
            }

            mPaint.setColor(Color.WHITE);
            mPaint.setStyle(Paint.Style.STROKE);
            canvas.drawLine(lineStartX,lineStartY,lineEndX,lineEndY,mPaint);
            if(lineAngel >90 && lineAngel <270){//圆的左半部分
